...
function ConnectionsUtility:AddConnection(Connection: RBXScriptConnection, Path: string, Replace: boolean)
...
function ConnectionsUtility:GetConnection(Path: string)
...
function ConnectionsUtility:RemoveConnection(Path: string)
...
function ConnectionsUtility:RemoveAllConnections()
	-- Simply iterates through each connection and disconnects it, then removes it from the table.
	for _, Connection in pairs(Connections._ActiveConnections) do
...
function ConnectionsUtility:RemoveConnectionsFromPath(Path: string)
	local ConnectionsToRemove = {}

	-- Uses the `Path` parameter to target and disconnect all connections that match the path.
	for ConnectionPath in Connections._ActiveConnections do
	...
	for ConnectionPath in ConnectionsToRemove do
